...
int getIntInput(const char *prompt, int min_val) {
    int value;
    char buffer[100]; // Temporary buffer for reading line
    while (1) {
        printf("%s: ", prompt); // Add a colon for consistency
        if (fgets(buffer, sizeof(buffer), stdin) == NULL) {
            // Handle EOF or error
            return 0; // Or appropriate error code
        }
        // Attempt to parse the integer from the buffer
        if (sscanf(buffer, "%d", &value) == 1 && value >= min_val) {
            // Check if there are extra characters after the integer (indicates invalid input)
            char remainder[2]; // To check for any leftover characters
            if (sscanf(buffer, "%d%1s", &value, remainder) != 2) { // Try to read integer and then one non-whitespace char
                return value;
            }
        }
        printf("Invalid input. Please enter a valid number (>= %d).\n", min_val);
    }
}


float getFloatInput(const char *prompt, float min_val) {
    float value;
    char buffer[100]; // Temporary buffer for reading line
    while (1) {
        printf("%s: ", prompt);
        ...
        if (sscanf(buffer, "%f", &value) == 1 && value >= min_val) {
             char remainder[2];
            if (sscanf(buffer, "%f%1s", &value, remainder) != 2) {
                return value;
            }
        }
        printf("Invalid input. Please enter a valid number (>= %.2f).\n", min_val);
    }
}

void getStringInput(const char *prompt, char *buffer, int max_len) {
    printf("%s", prompt); // The prompt itself
    // Use fgets to read the string
    if (fgets(buffer, max_len, stdin) != NULL) {
        // Remove the trailing newline character, if present
        buffer[strcspn(buffer, "\n")] = 0;
    } else {
        // Handle error or EOF if fgets returns NULL
        buffer[0] = '\0'; // Set to empty string
    }
}

int generateVoucherID() {
    static int nextVoucherId = 1235;
